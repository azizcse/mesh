...
import com.w3engineers.meshrnd.databinding.ActivityWalletBinding;
...
import com.w3engineers.purchase.ui.tokenguide.PointGuidelineActivity;
...
public class WalletActivity extends BaseActivity implements WalletManager.WalletListener {
    ...
    private void openUrl() {
       ...
       // if (myRole == DataPlanConstants.USER_ROLE.DATA_SELLER || myRole == DataPlanConstants.USER_ROLE.INTERNET_USER) {
            ...
            if (networkType == 1) { // ETH
                networkUrl = SharedPref.read(PurchaseConstants.GIFT_KEYS.TX_URL_ROPSTEN) + walletManager.getMyAddress();
            } else if (networkType == 2) { // ETC
                networkUrl = SharedPref.read(PurchaseConstants.GIFT_KEYS.TX_HISTORY_URL_KOTTI) + walletManager.getMyAddress();
            }
            if (networkUrl != null) {

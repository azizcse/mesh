...
import com.w3engineers.purchase.ui.util.ToastUtil;
...
//                int endPointType = preferencesHelperDataplan.getGiftEndpointType();
    ...
    public void buyData(double amount, String sellerAddress) {
        try {
            ...
            if (!payController.getTransportManagerX().isUserConnected(sellerAddress)) {
                if (dataPlanListener != null) {
...
//            String currentSellerId = payController.getTransportManagerX().getCurrentSellerId();
//
...
//            }
    ...
    public void getMyBalanceInfo() {
        ...
        if (NetworkMonitor.isOnline()){
            ...
            try {
                List<String> sellerIds = payController.getTransportManagerX().getInternetSellers();
                if (sellerIds.size() == 0) {
...
//                setCurrentSellerWithStatus(sellerId, PurchaseConstants.SELLERS_BTN_TEXT.CLOSING);
                ...
                if (ethBalance > 0) {
                    ...
                    purchase.state = PurchaseConstants.CHANNEL_STATE.OPEN;
                    databaseService.updatePurchase(purchase);
                    purchaseCloseFailed(purchase.sellerAddress, PurchaseManagerUtil.getCurrencyTypeMessage("You do not have enough %s."));
                }
            } else {
    ...
    private void cloasePurchaseWithSeller(String sellerId){
        ...
        try {
            if (payController.getTransportManagerX().isUserConnected(sellerId)) {
                receiverId = sellerId;
            } else {
                List<String> sellerIds = payController.getTransportManagerX().getInternetSellers();
                if (sellerIds.size() > 0) {
...
//                    setCurrentSellerWithStatus(sellerId, PurchaseConstants.SELLERS_BTN_TEXT.CLOSING);
    ...
    public void onUserConnected(String address) {
        MeshLog.v("onUserConnected at buyer " + address);
        try {
            if (payController.getTransportManagerX().isInternetSeller(address)) {
                MeshLog.v("the man is a seller " + address);
                DataPlanManager.getInstance().processAllSeller(mContext);

//                if (TextUtils.isEmpty(payController.getTransportManagerX().getCurrentSellerId()) && TextUtils.isEmpty(probableSellerId)){
...
//                }
                ...
                MeshLog.v("the man is not a seller " + address);
            }
...
//                        ethService.setMyEthBalance(ethBalance);
                        ...
                        if (ethBalance > PurchaseConstants.BUY_TOKEN_ETHER_VALUE) {
                            ...
                            if (walletListener != null) {
                                walletListener.onTokenRequestResponse(false, PurchaseManagerUtil.getCurrencyTypeMessage("You don't have enough %s."));
                            }
                        }
                    } else {
                ...
                try {
                    ...
                    if (!infoJson.has(PurchaseConstants.INFO_KEYS.ETH_BALANCE) || !infoJson.has(PurchaseConstants.INFO_KEYS.TKN_BALANCE)
                        ...
                        if (pendingRequestNumber > 0) {
                            ...
                            failedMessage = PurchaseManagerUtil.getCurrencyTypeMessage("You do not have enough %s.");
...
//                if (!sellerId.equalsIgnoreCase(p.sellerAddress) && payController.getTransportManagerX().isUserConnected(p.sellerAddress) && p.totalDataAmount > p.usedDataAmount){
...
//        }
    ...
    public void onDisconnectedBySeller(String sellerAddress, String msg) {
        //TODO need to review, define which routes should be offline, only those users who are connected by this user.
...
//            if (payController.getTransportManagerX().getCurrentSellerId().equalsIgnoreCase(sellerAddress)) {
            payController.getTransportManagerX().makeInternetUsersOffline(sellerAddress);
//            }
...
//            }
    ...
    @Override
    public void onSyncMyBuyerCalled() {
        syncMyPurchasesWithConnectedSellers();
    }
    ...
    public void onReceivedEtherRequestResponse(String from, int responseCode) {
        if (responseCode == 200) {
            ...
            if (walletListener != null) {
                walletListener.onEtherRequestResponse(true, PurchaseManagerUtil.getCurrencyTypeMessage("Request has been sent, %s will be added to your address soon."));
            }
        } else {

            if (walletListener != null) {
                walletListener.onEtherRequestResponse(false, PurchaseManagerUtil.getCurrencyTypeMessage("%s request rejected!"));
            }
        }
    }
    ...
    public void onPendingMessageInfo(String fromAddress, long dataSize, String msg_id, boolean isIncoming) {
        ...
        try {
            ...
            if (purchase != null) {
                ...
                MeshLog.v("Message id buyer " + msg_id  + "  purchase.balance " + purchase.balance);
                MeshLog.v("  bps_balance " + totalBalance +"  dataPrice " + dataPrice);
...
//                    });
                    ...
                    payController.getTransportManagerX().makeInternetUsersOffline(fromAddress);
...
//                MeshLog.p(" onInternetMessageResponseSuccess EX " + e.getMessage());
    ...
    private void sendSyncOkMessageToSeller(String sellerAddress, String bps, String trxHash){
        MeshLog.v("sendSyncOkMessageToSeller");
        JSONObject jsonObject = new JSONObject();
        try {
...
//                    if (typedResponse._receiver_address.equalsIgnoreCase(payController.getTransportManagerX().getCurrentSellerId())) {
                        payController.getTransportManagerX().makeInternetUsersOffline(typedResponse._receiver_address);
...
//                    }
                    ...
                    if (payController.getTransportManagerX().isUserConnected(typedResponse._receiver_address)){
                        JSONObject jsonObject = new JSONObject();
                        jsonObject.put(PurchaseConstants.JSON_KEYS.MESSAME_FROM, ethService.getAddress());
    ...
    public void syncMyPurchasesWithConnectedSellers(){
        try {
            List<Purchase> myOpenPurchases  =  databaseService.getMyPurchasesWithState(ethService.getAddress(), PurchaseConstants.CHANNEL_STATE.OPEN);

            //TODO looping may arise if there are multiple sellers connected, and buyer has multiple purchases, and no seller can connect the buyer as they finish their shared data.
            for (Purchase p : myOpenPurchases){
                if (payController.getTransportManagerX().isUserConnected(p.sellerAddress) && p.totalDataAmount > p.usedDataAmount){
                    JSONObject jsonObject = new JSONObject();
                    jsonObject.put(PurchaseConstants.JSON_KEYS.MESSAME_FROM, ethService.getAddress());
                    jsonObject.put(PurchaseConstants.JSON_KEYS.BUYER_ADDRESS, p.buyerAddress);
                    jsonObject.put(PurchaseConstants.JSON_KEYS.SELLER_ADDRESS, p.sellerAddress);
...
//    public boolean canSendFile(long fileSize) {
//        try {
//            long chunkNumber = (long) Math.ceil(fileSize/(10 * 1024)) + 1;
//            long dataSize = fileSize + chunkNumber * Constant.FILE_MESSAGE_ACK_SIZE;
//            double dataSizeMb = Util.convertBytesToMegabytes(dataSize);
//            double dataPrice = PreferencesHelperDataplan.on().getPerMbTokenValue() * dataSizeMb;
//
//            String currentSellerId = payController.getTransportManagerX().getCurrentSellerId();
//            Purchase purchase = databaseService.getPurchaseByState(PurchaseConstants.CHANNEL_STATE.OPEN, ethService.getAddress(), currentSellerId);
//            if (purchase != null) {
//                double totalBalance = purchase.balance + dataPrice;
//                if (totalBalance <= purchase.deposit) {
//                    return true;

...
public class PayController {
    ...
    private static PayControllerListenerForDataplanManager payControllerListenerForDataplanManager;
    ...
    PayController() {
        transportManagerX = TransportManagerX.getInstance();
        transportManagerX.setLinkStateListenerForPayment(linkStateListener);
    ...
    LinkStateListener linkStateListener = new LinkStateListener() {
        ...
        public void onTransportInit(String nodeId, String publicKey, TransportState transportState, String msg) {
            if (payControllerListenerForDataplanManager != null) {
                payControllerListenerForDataplanManager.onTransportInit(nodeId, publicKey, transportState, msg);
            }
        }
        ...
        public void onLocalUserConnected(String nodeId, String publicKey) {
            ...
            MeshLog.v("onLocalUserConnected " + nodeId);
...
//            sendUserConnected(nodeId, true);
        ...
        public void onMessagePayReceived(String sender, byte[] paymentData) {
            onPayMessageReceived(sender, paymentData);
        }

        @Override
        public void onPayMessageAckReceived(String sender, String receiver, String messageId) {
            onPayMessageAckReceived_(sender, receiver, messageId);
        }
        ...
        public void buyerInternetMessageReceived(String sender, String receiver, String messageId, String messageData, long dataLength, boolean isIncoming, boolean isFile) {
            if (payControllerListenerForSeller != null) {
                payControllerListenerForSeller.buyerInternetMessageReceived(sender, receiver, messageId, messageData, dataLength, isIncoming, isFile);
            }
        }

        @Override
        public void onProbableSellerDisconnected(String sellerId) {
            //We do not need this as multiple sellers are connected in parallel
//            if (payControllerListenerForBuyer != null) {
//                payControllerListenerForBuyer.onProbableSellerDisconnected(sellerId);
//            }
        }

        @Override
        public void onUserModeSwitch(String sendId, int newRole, int previousRole) {
            if (payControllerListenerForDataplanManager != null && previousRole != -1) {
                payControllerListenerForDataplanManager.onUserChooseNewRole(sendId, newRole, previousRole);
            }

            if (previousRole == DataPlanConstants.USER_ROLE.DATA_BUYER && PreferencesHelperDataplan.on().getDataPlanRole() == DataPlanConstants.USER_ROLE.DATA_SELLER){
                if (payControllerListenerForSeller != null) {
                    payControllerListenerForSeller.onBuyerChangedRole(sendId);
                }
            }
        }
    ...
    public interface PayControllerListenerForBuyer {
        void onInitPurchaseOkReceived(String sellerAddress, double ethBalance, double tokenBallance,
        ...
        void onInternetMessageResponseSuccess(String sender, String bps, double bps_balance, String chs,
                                              long open_block, long byteSize, String messageId);
    ...
    public interface PayControllerListenerForSeller {
        ...
        void buyerInternetMessageReceived(String sender, String owner, String msg_id, String msgData, long dataSize, boolean isIncomming, boolean isFile);
        ...
        void onBuyerChangedRole(String buyerId);
    }

    public interface PayControllerListenerForDataplanManager {
        void onTransportInit(String nodeId, String publicKey, TransportState transportState, String msg);

        /**
         * <h>User role switch receiver</h1>
         * When a local user switch user mode
         * This method responsible for ui update
         *
         * @param userId (Not required) : String
         */
        void onUserChooseNewRole(String userId, int newRole, int previousRole);
    }
...
//    }
    ...
    public TransportManagerX getTransportManagerX() {
    ...
    private void onPayMessageReceived(String sender, byte[] paymentData) {
        String receiveMsg = new String(paymentData);

        try {
            String userPublicKey = transportManagerX.getUserPublicKey(sender);
...
//                        break;
                    ...
                    case PurchaseConstants.MESSAGE_TYPES.SYNC_SELLER_TO_BUYER_OK:
                        MeshLog.v("MESSAGE_TYPES.SYNC_SELLER_TO_BUYER_OK");
                        sellerAddress = jsonObject.getString(PurchaseConstants.JSON_KEYS.SELLER_ADDRESS);
                        bps = jsonObject.getString(PurchaseConstants.JSON_KEYS.MESSAGE_BPS);
                        trx_hash = jsonObject.getString(PurchaseConstants.JSON_KEYS.TRX_HASH);
    ...
    private void onPayMessageAckReceived_(String sender, String receiver, String messageId) {
    ...
    private void sendUserConnected(String address, boolean isConnected) {
        ...
        MeshLog.v("sendUserConnected "+ address + " userMode " + userMode);
    ...
    private void sendPayMessageToTransport(String address, String message, String messageId) {
        ...
        try {
            String userPublicKey = transportManagerX.getUserPublicKey(address);
            if (!TextUtils.isEmpty(userPublicKey)) {
                String encryptedMessage = CryptoHelper.encryptMessage(walletService.getPrivateKey(), userPublicKey, message);
                transportManagerX.sendPayMessage(address, encryptedMessage, messageId);
            } else {
                MeshLog.v("User public not found");
            }
...
//        sendPayMessage(receiver, jsonObject.toString());
    ...
    public void setPayControllerListenerForDataplanManager(PayControllerListenerForDataplanManager payControllerListenerForDataplanManager_) {
        payControllerListenerForDataplanManager = payControllerListenerForDataplanManager_;
    }

    public void syncMyBuyer(String sellerId){
        if (sellerId != null){
            if (payControllerListenerForBuyer != null) {
                payControllerListenerForBuyer.onUserConnected(sellerId);
            }
        } else {
            if (payControllerListenerForBuyer != null) {
                payControllerListenerForBuyer.onSyncMyBuyerCalled();
            }
        }
    }
}
